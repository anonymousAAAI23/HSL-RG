...
def sample_meta_datasets(data, dataset, task, n_shot, n_query):
    ...
    #print(s_list_1)
    #print(data[0,1])
    s_adapt = [data[i] for i in s_list_1]
    q_adapt = [data[i] for i in q_list_1]
...
def sample_test_datasets(data, dataset, task, n_shot, n_query, update_step=1):
    ...
    s_data = [data[i] for i in s_list]
    q_data = [data[i] for i in q_list]
    #s_adapt = data[torch.tensor(s_list)]
    #q_adapt = data[torch.tensor(q_list)]

    q_sample = sample_inds(q_list, update_step * n_query)
    #q_data_adapt = data[torch.tensor(q_sample)]
    q_data_adapt = [data[i] for i in q_sample]
