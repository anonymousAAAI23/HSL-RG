...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if not empty:
            ...
            self.smiles_list = pd.read_csv(os.path.join(self.processed_dir,'smiles.csv'),header=None).values[:,0]
    ...
    def process(self):
        ...
        if self.dataset == 'tox21':
            ...
            #if not os.path.exists("kernel.npy"):
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    # manually add mol id
                    data.id = torch.tensor(
                        [k])  # id here is the index of the mol in
                    # the dataset
                    k += 1
                    data.y = torch.tensor(labels[i, :])
                    data_list.append(data)
