...
class ContextMLP(nn.Module):
    ...
    def forward(self, s_emb, q_emb):
        ...
        return all_emb, None
        '''
class NodeUpdateNetworkOld(nn.Module):
    def __init__(self, inp_dim, out_dim, n_layer=2, batch_norm=False, dropout=0.0):
        super(NodeUpdateNetworkOld, self).__init__()
        # set size
...
class NodeUpdateNetworkGCN(nn.Module):
    def __init__(self, inp_dim, out_dim, n_layer=2, batch_norm=False, dropout=0.0):
        super(NodeUpdateNetwork, self).__init__()
        self.layer1 = nn.Linear(inp_dim, inp_dim)
        self.layer2 = nn.Linear(inp_dim, out_dim)
        self.relu = nn.LeakyReLU()

    def forward(self, node_feat, edge_feat):
        n_query = edge_feat.size(0)
        n_nodes = edge_feat.size(1)
        self_loop = torch.eye(n_nodes).unsqueeze(0).repeat(n_query, 1, 1).to(edge_feat.device)
        diag_mask = 1.0 - self_loop
        edge_feat = edge_feat * diag_mask + self_loop
        #print(torch.sum(edge_feat, 2))
        degree = torch.diag_embed(torch.sum(edge_feat, 2).pow(-0.5))
        #print(degree)
        transform = torch.bmm(torch.bmm(degree, edge_feat), degree)
        #print("node_feat0:", torch.sum(node_feat))
        #print("transform:", torch.sum(transform))
        node_feat = torch.bmm(transform, node_feat)
        #print("node_feat1:", torch.sum(node_feat))
        node_feat = self.relu(self.layer1(node_feat))
        node_feat = torch.bmm(transform, node_feat)
        #print("node_feat2:", torch.sum(node_feat))
        node_feat = self.layer2(node_feat)
        #print("node_feat3:", torch.sum(node_feat))
        return node_feat
...
class NodeUpdateNetwork(nn.Module):
    def __init__(self, inp_dim, out_dim, n_layer=2, batch_norm=False, dropout=0.0):
        ...
        mlp1 = nn.Sequential(nn.Linear(inp_dim, inp_dim), nn.ReLU())
        self.gnns.append(GINEConv(mlp1, edge_dim=1))
        #self.gnns.append(GINConv(mlp1))
        mlp2 = nn.Sequential(nn.Linear(inp_dim, inp_dim), nn.ReLU())
        self.gnns.append(GINEConv(mlp2, edge_dim=1))
        #self.gnns.append(GINConv(mlp2))
        self.batch_norm = batch_norm
        if self.batch_norm:
            self.batch_norms = torch.nn.ModuleList()
            #self.batch_norms.append(torch.nn.BatchNorm1d(inp_dim))
            self.batch_norms.append(torch.nn.BatchNorm1d(out_dim))
        self.dropout = dropout
        self.num_layer = n_layer
        self.network = nn.Conv2d(
                in_channels=inp_dim*2,
    ...
    def forward(self, node_feat, edge_feat):
        n_query, n_nodes, n_feat = node_feat.size()
        #if True:
        
        if torch.sum(edge_feat) == 0:
            self_loop = torch.eye(n_nodes).unsqueeze(0).repeat(n_query, 1, 1).to(edge_feat.device)
            diag_mask = 1.0 - self_loop
            edge_feat = edge_feat * diag_mask + self_loop
        
        h = node_feat.reshape(n_query*n_nodes, -1)
        #edge_feat = edge_feat.reshape(n_query*n_nodes, -1).repeat(1, n_query)
        #self_loop = torch.eye(n_nodes).repeat(n_query, n_query).to(edge_feat.device)
        #diag_mask = 1.0 - self_loop
        #edge_feat = edge_feat * diag_mask + self_loop
        edge_index, edge_attr = dense_to_sparse(edge_feat)
        #print(edge_index)
        #print(edge_attr)
        ...
        node_feat = torch.cat([h.reshape(n_query, n_nodes, -1), node_feat], -1).transpose(1, 2)
        #node_feat = torch.cat([node_feat, node_feat], -1).transpose(1, 2)
        node_feat = self.network(node_feat.unsqueeze(-1)).transpose(1, 2).squeeze(-1)
...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        self_loop_attr[:, 0] = 1  # bond type for self-loop edge
...
'''    
class GNN(nn.Module):
    def __init__(self, inp_dim, n_layer=5, batch_norm=False, dropout=0.5):
        super(GNN, self).__init__()
        self.gnns = torch.nn.ModuleList()
        for layer in range(n_layer):
            self.gnns.append(GINConv(inp_dim, aggr="add"))
    ...
    def forward(self, node_feat, edge_feat):
        ...
        for layer in range(self.num_layer):
            ...
            if layer == self.num_layer - 1:
                ...
                h = F.dropout(F.relu(h), self.dropout)
        node_feat = h.reshape(n_query, n_nodes, -1)
        return node_feat
    
class NodeUpdateNetworkPretrained(nn.Module):
    def __init__(self, inp_dim, n_layer=5, batch_norm=False, dropout=0.5):
        super(NodeUpdateNetwork, self).__init__()
        self.gnn = GNN(inp_dim, n_layer, batch_norm, dropout)
        
    def from_pretrained(self, model_file, gpu_id):
        if torch.cuda.is_available():
            pretrained_dict = torch.load(model_file, map_location='cuda:' + str(gpu_id))
    ...
    def forward(self, node_feat, edge_feat):
        node_feat = self.gnn(node_feat, edge_feat)
        return node_feat
...
class TaskAwareRelation(nn.Module):
    def __init__(self, inp_dim, hidden_dim, num_layers, edge_n_layer, node_n_layer=2, num_class=2,
                res_alpha=0., top_k=-1, node_concat=True, batch_norm=False, dropout=0.0,
                 adj_type='sim', activation='softmax',pre_dropout=0.0,model_file=None,gpu_id=None):
        ...
        for i in range(self.num_layers):
            ...
            '''
            module_l = NodeUpdateNetworkOld(inp_dim=gnn_inp_dim, out_dim=hidden_dim, n_layer=node_n_layer,
            ...
            #module_l.from_pretrained(model_file, gpu_id)
        ...
        #self.fc2 = nn.Linear(gnn_inp_dim, num_class)

        assert 0 <= res_alpha <= 1

    def forward(self, all_emb, q_emb=None, return_adj=False, return_emb=False, adj=None):
        node_feat=all_emb
        if self.pre_dropout>0:
            node_feat=self.predrop1(node_feat)
        '''
        adj = adj.unsqueeze(1)
        n_q, n_edge, n1, n2 = adj.size()
        adj = adj.reshape(n_q * n_edge * n1, n2)
        ...
        #pool_node_feat = torch.mean(node_feat, 1) 
        #pool_node_feat = torch.mean(pool_node_feat, 0)

        s_feat = node_feat[:, :-1, :]
        n_shot = s_feat.size(1) // 2
        neg_feat = torch.mean(s_feat[:, :n_shot, :], 1)
        pos_feat = torch.mean(s_feat[:, n_shot:, :], 1)
        pool_node_feat = torch.cat((neg_feat, pos_feat), 1)
        pool_node_feat = torch.mean(pool_node_feat, 0)
        q_feat = node_feat[:, -1, :]
        ...
        if return_emb:
            return s_logits, q_logits, edge_feat_list, s_feat, q_feat, pool_node_feat
        else:
            return s_logits, q_logits, edge_feat_list, pool_node_feat
